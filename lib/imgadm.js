...
/**
 * Snapshots a given image.
 *
 *
 * @param {Object} options:
 *      - @param {UUID} uuid - The UUID of the image
 *      - @param {UUID} name - The name of the snapshot
 *      - @param {Object} log - A log object on which to call log.info
 *        for successful run output.
 * @param callback {Function} `function (err)`
 */
function snapshotImage(opts, callback) {
    assert.ok(opts, 'options');
    assert.ok(opts.uuid && UUID_RE.test(opts.uuid), 'options.uuid');
    assert.ok(opts.name, 'options.name');
    assert.ok(opts.log, 'options.log');

    var snapName = format('zones/%s@%s', opts.uuid, opts.name);
    var argv = ['/usr/sbin/zfs', 'snapshot', snapName];

    opts.log.info('calling: ' + argv.join(' '));
    execFile(argv[0], argv.slice(1), function (err, stdout, stderr) {
        if (err) {
            callback(new Error(format(
                'Error taking snapshot %s: %s', opts.uuid, stderr.trim())));
            return;
        }
        opts.log.info(format(
            'took snapshot of image %s: stdout=%s stderr=%s',
            opts.uuid, stdout.trim(), stderr.trim()));
        callback(null);
    });
}
...
/**
 * Sends a `zfs send` stream to a calling writable stream.
 *
 *
 * @param {Object} options:
 *      - @param {UUID} uuid - The UUID of the image
 ...
 * @param callback {Function} `function (err)`
 */
function sendImage(opts, callback) {
    assert.ok(opts, 'options');
    assert.ok(opts.uuid && UUID_RE.test(opts.uuid), 'options.uuid');
    assert.ok(opts.name, 'options.name');
    assert.ok(opts.stream, 'options.stream');
    assert.ok(opts.log, 'options.log');

    var snapName = format('zones/%s@%s', opts.uuid, opts.name);
    var argv = ['send', snapName];

    opts.log.info('spawning: zfs ' + argv.join(' '));

    var zfsSend = spawn('/usr/sbin/zfs', argv);
    ...
    zfsSend.on('exit', function (code, signal) {
        opts.log.info({
            argv: argv,
            code: code,
            signal: signal
    ...
    zfsSend.on('close', function (code) {
        opts.log.info({ argv: argv }, 'zfs send closed');
        ...
        if (code !== 0) {
            ...
            opts.log.info({argv: argv}, 'zfs send exited successfully');
            callback();
        }
    });
