...
const LinuxVmadm = require('vmadm/lib/index.linux');
...
function start(callback) {
    ...
    function (waterfallError) {
        ...
        self.vmadm.load(opts, vmopts, function (loadError, machine) {
            if (waterfallError) {
                if (machine) {
...
function pre_check(callback) {
    ...
    self.vmadm.exists(opts, function existsCb(err, exists) {
        if (err) {
            callback(err);
            return;
        }
        if (exists) {
            callback(new Error('Machine ' + self.req.params.uuid + ' exists.'));
            return;
        }
        callback();
    });
...
function ensure_dataset_present(callback) {
    ...
    fullDataset = self.req.params.zpool + '/' + self.toImport;
