...
function _getTritonNetworkingConfig() {
    if (!_netConfig) {
        _netConfig = JSON.parse(fs.readFileSync(
...
function getAgents(callback) {
    const AGENTS_PATH = '/opt/smartdc/agents/lib/node_modules';
    const SDC_Agents = [];
    fs.readdir(AGENTS_PATH, function (err, agents) {
        if (err) {
            callback(err);
            return;
        }
        vasync.forEachParallel({
            ...
            func: function (agent, next) {
                try {
                    var agentJSON = require(AGENTS_PATH + '/' +
                                    agent + '/package.json');
                    SDC_Agents.push({
                        name: agentJSON.name,
...
function getBasicInfo(opts, callback)
{
    ...
    var configPrefix;
    var setupStateFile;
    if (fs.existsSync('/opt/triton/config')) {
        configPrefix = '/opt/triton/config';
    } else {
        configPrefix = '/usr/triton/config';
    }

    setupStateFile = configPrefix + '/triton-setup-state.json';

    if (fs.existsSync(setupStateFile)) {
        try {
            var setupContents = fs.readFileSync(setupStateFile);
    ...
    getNodeConfigInfo(opts, function _onGetConfigCb(err, config) {
        ...
        getAgents(function getAgentsCb(getErr, agents) {
            if (getErr) {
                if (sysinfo.Setup === true) {
                    console.warn('Unable to get agents: ' + getErr);
                }
                callback(null, sysinfo);
                return;
            }
            sysinfo['SDC Agents'] = agents;
            callback(null, sysinfo);
        });
    });
}

function getSystemType(opts, callback) {
    execFile('/bin/uname', ['-s'], function _unameCb(unameErr, stdout, stderr) {
        if (unameErr) {
            callback(unameErr, {'stdout': stdout, 'stderr': stderr});
            return;
        }

        var sysinfo = {
            'System Type': stdout.split('\n')[0]
        };
        callback(null, sysinfo);
    });
}
 ...
 * 'CPU Core Count'         Number of physical CPU cores in this host.
 */
...
function getNetInfo(opts, callback)
{
    ...
    var adminNicTag = netConfig.admin_tag || 'admin';
    var bootParams = opts.bootParams;

    // Generate the list of NIC tag names for each MAC.
    var nicTagNamesForMac = {};
    (netConfig.nictags || []).forEach(function _eachNicTag(nicTagObj) {
        var names = nicTagNamesForMac[nicTagObj.mac];
        if (!names) {
            names = [nicTagObj.name];
            nicTagNamesForMac[nicTagObj.mac] = names;
        } else {
            names.push(nicTagObj.name);
        }
    });
    ...
    execFile('/sbin/ip', ['-json', 'addr'], function ipA(err, stdout, stderr) {
        ...
        for (var i = 0; i < nets.length; i++) {
            ...
            // Only do physical interfaces for now because we don't yet support
            // aggrs (bond) or vnics (veth, vlan, bridges, etc).
            // aggr/bond interfaces should show up in nics, but vnic/veth/vlan
            // interfaces need to go in vnics.
            // Where should bridges go? Crossbow does bridges, but it's a
            // completely different use cas that doesn't come up in Triton.
            // Linux on the other hand seems to use bridges as their go-to
            // virtual network constructor
            var device_dir_path = '/sys/class/net/' + net.ifname +
                '/device/device';
            if (!fs.existsSync(device_dir_path)) {
                continue;
            }
            ...
            nics[net.ifname] = {
                ...
                'ip4addr': '',
                'NIC Names': (nicTagNamesForMac[net.address] || [])
            };
            ...
            for (var j = 0; j < net.addr_info.length; j++) {
                ...
                if (adminip && adminip === addr.local &&
                        nic['NIC Names'].indexOf(adminNicTag) === -1) {
...
function swapUuidBytes(uuid) {
    /*
     * Return a byte swapped UUID.
     *
     * In some cases (e.g., SMBIOS) the first three fields of a UUID
     * are big or little endian and you need the opposite. This function
     * doesn't know or care what the correct endian-ness is. But if you
     * have one and you know you need to reverse it, we'll reverse it for you.
     *
     */
    var i, x;
    x = uuid.split('-');
    for (i = 0; i < 3; i++) {
        x[i] = x[i].match(/(.{1,2})/g).reverse().join('');
    }
    return (x.join('-'));
}
...
function getSmbiosInfo(opts, callback)
{
    execFile('/usr/sbin/dmidecode', ['-t', '1'],
        ...
        if (sysinfo['UUID']) {
            sysinfo['UUID'] = swapUuidBytes(sysinfo['UUID'].toLowerCase());
        }
