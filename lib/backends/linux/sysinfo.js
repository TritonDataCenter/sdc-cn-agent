...
function getBasicInfo(log, callback)
{
    ...
    // TODO: Why have two files for the same thing?
    if (fs.existsSync('/usr/triton/state/setup_complete')) {
        sysinfo.Setup = true;
    } else {
        try {
            var setupContents = fs.readFileSync('/usr/triton/state/triton-setup.json');
...
function getNetInfo(log, callback)
{
    ...
    execFile('/sbin/ip', ['-json', 'addr'], function ipA(err, stdout, stderr) {
        ...
        var aggrs = {};
        var aggr_ifs = {};
        // XXX-mg hack until nic tags are working properly
        var adminip = process.env.ADMIN_IP;
