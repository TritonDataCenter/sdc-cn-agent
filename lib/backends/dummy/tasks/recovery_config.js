...
function start() {
    ...
    var kbmapiaddr;
    var server_uuid;
    var guid = self.request.params.pivtoken;
    var template = self.request.params.template;
    var action = self.request.params.action;
    var recovery_uuid = self.request.params.recovery_uuid;
    var token = self.request.params.token;
    var templateFile;
    var zpoolRecovery = {};
    ...
    vasync.waterfall([
        ...
        function bailIfMissingPreconditions(cb) {
            ...
            if (action === 'activate' &&
                zpoolRecovery.staged !== recovery_uuid) {
        ...
        function doAction(cb) {
            ...
            if (action === 'activate') {
                ...
                args.push('add', '-t', templateFile, '-r', token);
            }
            ...
            if (action === 'activate') {
                ...
                zpoolRecovery.staged = null;
            } else {
                zpoolRecovery.staged = sha512str(template);
            }
            self.progress(65);
            cb();
        },
        ...
        function postUpdatesToKbmapi(cb) {
            ...
            Object.keys(zpoolRecovery).forEach(function uuidfromstr(k) {
                params.zpool_recovery[k] =
                    repeatableUUIDFromString(zpoolRecovery[k]);
            });

            if (token) {
                params.recovery_token =
                    repeatableUUIDFromString(sha512str(token));
            }
