...
function runContainerLogsCommand(container, params, socket) {
    ...
    lstream.on('readable', function runContainerLogsCommandOnReadable() {
        ...
        while ((line = lstream.read()) != null) {
            ...
            // older logs are JSON formatted with the content in the 'log'
            // property, newer logs are stored as bunyan formatted messages
            // where the content is in the 'msg' property
            if (rec.hasOwnProperty('log')) {
                data = rec.log;
            } else if (rec.hasOwnProperty('msg')) {
                data = rec.msg;
            } else {
                continue;
            }
