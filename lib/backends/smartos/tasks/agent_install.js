    ...
    async.waterfall([
        function getSdcConfigImgapiDomain(cb) {
            ...
            smartdc_config.sdcConfig(function (configError, config) {
                ...
                self.log.info({elapsed: elapsed(_start)},
                    'smartdc_config.sdcConfig');
        ...
        function installPackage(cb) {
            ...
            apm.installPackages([package_file], function (err) {
                self.log.info({elapsed: elapsed(_start)}, 'apm install');
        ...
        function sendAgentsToCNAPI(cb) {
            ...
            if (!do_update || (is_update && package_name !== 'cn-agent')) {
                self.log.warn({
                    do_update: do_update,
                    is_update: is_update,
                    package_name: package_name,
        ...
        self.log.info({elapsed: elapsed(taskStart)},
            '[TASK] agent_install');
