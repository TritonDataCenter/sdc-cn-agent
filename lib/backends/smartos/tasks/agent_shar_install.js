...
function AgentSharInstallTask(req) {
    Task.call(this);
    this.req = req;
}

Task.createTask(AgentSharInstallTask);
...
function start() {
    ...
    }, 'would try to install agentshar');

    self.progress(100);
    return self.finish();
}

AgentSharInstallTask.setStart(start);

module.exports = AgentSharInstallTask;
