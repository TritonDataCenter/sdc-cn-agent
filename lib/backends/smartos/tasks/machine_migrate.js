...
/*
 * Actions handled by the bhyve live-migration child script.  Each of
 * these is a thin forwarder to the per-CN vmm-migrate-agent's local
 * Unix socket — see machine-migrate-live.js for details.
 */
var LIVE_ACTIONS = {
    'live-begin':  true,
    'live-sync':   true,
...
function startChildProcess() {
    ...
    } else if (LIVE_ACTIONS[payload.action]) {
        binfn = __dirname + '/../bin/machine-migrate-live.js';
    }
...
function restoreZfsQuota() {
    ...
    // Quota restore has to cover the dataset's post-migration shape:
    //
    //   - The ORIGINAL quota (what the operator signed up for).
    //     Preferred when the dataset still fits under it.
    //   - The current `used` bytes PLUS a buffer so zfs accepts the
    //     quota change.  Migration snapshots (@mig-base,
    //     @mig-sync-NNN, @mig-final) can push `used` above what the
    //     original quota allowed, which is why the naive
    //     "quota = old-reservation" approach blew up with "size is
    //     less than current used or reserved space".
    //
    // Compute quota = max(original_quota, current_reservation,
    //                     current_used + QUOTA_BUFFER_BYTES).
    //
    // The max() gives a self-healing value: on the happy path
    // original_quota wins (no-op); on a snapshot-heavy source we
    // grow by used+buffer until the stale snapshots are reaped by
    // migrate-finalize, at which point an operator can shrink the
    // quota back (or, better, run vmadm update which recomputes).
    var QUOTA_BUFFER_BYTES = 1024 * 1024 * 1024; // 1 GiB

    var migRecord = payload.migrationTask && payload.migrationTask.record;
    var originalQuotaBytes = 0;
    if (migRecord) {
        // Which side are we on?  The record has source/target halves;
        // pick the one whose server_uuid matches this CN.  Fall back
        // to whichever is present.
        var sfsd = migRecord.sourceFilesystemDetails;
        var tfsd = migRecord.targetFilesystemDetails;
        var cand;
        if (sfsd && sfsd.quotaStr) {
            cand = parseInt(sfsd.quotaStr, 10);
            if (Number.isFinite(cand) && cand > 0) {
                originalQuotaBytes = cand;
            }
        }
        if (tfsd && tfsd.quotaStr && !originalQuotaBytes) {
            cand = parseInt(tfsd.quotaStr, 10);
            if (Number.isFinite(cand) && cand > 0) {
                originalQuotaBytes = cand;
            }
        }
    ...
    vasync.pipeline({arg: {}, funcs: [
        function getCurrentSizes(ctx, next) {
            var cmd = '/usr/sbin/zfs';
            var args = [
                'list', '-Hp', '-o', 'reservation,used',
                vm.zfs_filesystem
            ];

            log.debug({cmd: cmd, args: args}, 'restoreZfsQuota: zfs list');

            child_process.execFile(cmd, args, gExecFileDefaults,
            function _execZfsListCb(err, stdout, stderr) {
                if (err) {
                    log.error('zfs list error:', err, ', stderr:', stderr);
                ...
                var values = stdout.trim().split('\t');
                ctx.reservationBytes = parseInt(values[0], 10);
                if (!Number.isFinite(ctx.reservationBytes)) {
                    ctx.reservationBytes = 0;
                }
                ctx.usedBytes = parseInt(values[1], 10);
                if (!Number.isFinite(ctx.usedBytes)) {
                    ctx.usedBytes = 0;
                }
                next();
            });
        ...
        function computeAndSetQuota(ctx, next) {
            var usedPlusBuffer = ctx.usedBytes + QUOTA_BUFFER_BYTES;
            var targetQuota = Math.max(
                originalQuotaBytes,
                ctx.reservationBytes,
                usedPlusBuffer
            );

            if (!targetQuota || targetQuota <= 0) {
                log.info({
                    original: originalQuotaBytes,
                    reservation: ctx.reservationBytes,
                    used: ctx.usedBytes
                }, 'restoreZfsQuota: no positive quota candidate, ' +
                   'leaving quota unset');
                next();
                return;
            }
            ...
            log.info({
                zfs_filesystem: vm.zfs_filesystem,
                target_quota: targetQuota,
                original: originalQuotaBytes,
                reservation: ctx.reservationBytes,
                used: ctx.usedBytes,
                buffer: QUOTA_BUFFER_BYTES
            }, 'restoreZfsQuota: setting quota');

            child_process.execFile(cmd, args, gExecFileDefaults,
                    function _execZfsSetQuotaCb(err, stdout, stderr) {
                if (err) {
                    // Still best-effort: even the max-of-three
                    // computation can miss if a concurrent snapshot
                    // pushes used between list and set.  A wrong
                    // quota is an operator annoyance, not a reason
                    // to re-fail a migration that already cut over.
                    //
                    // The QUOTA_RESTORE_FAILED marker is stable for
                    // operator log-scraping — if a migration finishes
                    // "successful" but this string is in cn-agent logs
                    // for the CN, an operator knows to run
                    //   vmadm update <uuid> quota=<new-value>
                    // to set a consistent quota.
                    log.warn({
                        err: err,
                        ...
                        zfs_filesystem: vm.zfs_filesystem,
                        target_quota: targetQuota
                    }, 'QUOTA_RESTORE_FAILED: zfs set failed, ' +
                       'leaving quota unset (operator can fix with ' +
                       'vmadm update <uuid> quota=...)');
                } else {
                    log.info({
                        vm_uuid: vm.uuid,
                        zfs_filesystem: vm.zfs_filesystem,
                        quota: targetQuota
                    }, 'restoreZfsQuota: applied');
                }
                next();
            });
        }
        ...
        if (err) {
            log.warn({err: err},
                'restoreZfsQuota: aborting quota restore (non-fatal)');
        }
...
function start() {
    ...
    /*
     * Live migration (bhyve).  All four live-* actions share the same
     * dispatch entry point — startChildProcess resolves the child
     * binary to machine-migrate-live.js via the LIVE_ACTIONS table.
     */
    } else if (LIVE_ACTIONS[payload.action]) {
        startChildProcess.bind(this)();
