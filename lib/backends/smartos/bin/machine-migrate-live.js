 ...
 *   in:  { payload: { migrationTask: { action }, ... }, req_id, uuid }
...
function pollUntilTerminal(vm_uuid, deadline, log, callback) {
    function tick() {
        if (Date.now() > deadline) {
            callback(new Error('switch timed out waiting for terminal state'));
            return;
        }
        agentCall({command: 'status', vm: vm_uuid}, log,
            ...
            log.debug({state: state, phase: resp && resp.phase}, 'poll');
...
process.on('message', function (message) {
    ...
    var action = message.payload.migrationTask.action;
